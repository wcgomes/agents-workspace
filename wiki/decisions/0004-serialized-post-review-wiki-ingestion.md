...
Accepted (2026-08-04); amended (2026-08-13) to permit relevant, in-scope executor consultation while preserving the original write and ingestion decisions; further amended to apply the Wiki gate to automatic consultation and evaluation, preserving explicit wiki requests.
...
Subject to the boot policy's [Wiki gate](../../templates/AGENTS.md#coordinator-flow) and its explicit-request exception, after domain-appropriate review and verification and before the final response, the coordinator evaluates the reviewed artifacts, outcomes, corrections, and optional signals for uncaptured durable workspace knowledge. Evaluation remains domain-neutral; ingestion is conditional and never creates a wiki as an automatic side effect.
