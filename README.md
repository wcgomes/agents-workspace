...
- **Structured handoffs:** Every initial handoff for a fresh delegated conversation starts with `Session role: DELEGATED_SUBAGENT` and carries `Task`, `Objective`, `Scope`, `Done criteria`, `Constraints`, `Context`, `Deliverable`, `Compaction rule`, and `Return format`, plus optional `Spec ref` and specialist `Role` only for adjacent or fallback matches. Selected-agent, match-type, and fallback rationale stay coordinator-internal.
